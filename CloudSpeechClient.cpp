...
String CloudSpeechClient::getVoiceText_berry(String report_address, int port, String txt, String fn)
{
  //0D CR 归位键  \r
  //0A LF 换行    \n
  //uploadfile(fn);
  ...
  if (!client.connect(report_address.c_str(), port)) {
#ifdef SHOW_DEBUG
  ...
  //以下信息通过 Fiddler 工具分析协议，Raw部分获得  (此处要 //index? ）
  String url = String("http://") + report_address + ":" + String(port) + "/method=mp3_text&txt=" + txt;
  String  HttpHeader = String("GET ") + String(url) +
                       " HTTP/1.1\r\nHost: " + report_address + ":" + String(port) + "\r\nConnection: keep-alive" +
                       "\r\n\r\n";
#ifdef SHOW_DEBUG
  ...
  String retstr = "";
  String line = "";
  unsigned long all_starttime = 0;
  unsigned long all_stoptime = 0;
  ...
  delay(10);
  client.stop();
  all_stoptime = millis() / 1000;
#ifdef SHOW_DEBUG
  Serial.println(String("识别用时:") +  String(all_stoptime - all_starttime) + "秒");
#endif
  return (retstr);
}
